...
final class IzifootAPI {
    ...
    func updateActiveTeam(teamID: String) async throws -> Me {
        struct UpdateActiveTeamPayload: Encodable {
            let teamId: String
            let team_id: String
        }
        ...
        return try await client.put(
            APIRoutes.meTeam,
            body: UpdateActiveTeamPayload(teamId: teamID, team_id: teamID),
    ...
    func updateTraining(
        ...
        return try await client.put(
            ...
            body: UpdateTrainingPayload(status: status, date: dateISO8601, endTime: endTime),
            responseType: Training.self
        )
    }
